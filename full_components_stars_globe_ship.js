...
var logoMesh;
var ship, ship_thrusters, thrustBlasts, centerThrust, asteroids;
...
function initGlobe() {
  ...
  console.log("🚀 ~ initGlobe ~ Globe:", Globe)
...
function initModel() {
  ...
  loader.load(
    ...
    function (gltf) {
      ...
      const rocket_ship = new THREE.Group();
      ...
      rocket_ship.position.set(xPos, yPos, zPos);
      rocket_ship.name = "Rocket_ship";
      scene.add(rocket_ship);
...
function gsapAnimate() {
  ...
  // * GLOBE
  const globeMeshProps = {scale:1, rotation:degToRad(360)}
