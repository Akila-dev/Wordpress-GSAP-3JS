...
function init() {
  ...
  //  * CAMERA
  camera = new THREE.PerspectiveCamera(75, camera_aspect, 0.1, 1000);
...
function initParticles() {
  ...
  console.log("🚀 ~ initParticles ~ geometry:", geometry);
  ...
  for (var i = 0; i < no_of_partices * 3; i += 3) {
    ...
    posArray[i] = distance * Math.sin(theta) * Math.cos(phi) * 2;
    posArray[i + 1] = distance * Math.sin(theta) * Math.sin(phi) * 2;
    posArray[i + 2] = distance * Math.cos(theta) * 2;
  }
...
function gsapAnimate() {
  ...
  gsap.to(props, {
    ...
    ease: "none",
    onUpdate: function () {
      particlesGroup.rotation.set(props.xRot, props.xRot, 0);
    },
  });
}
